...
export default {
  ...
  methods: {
    async removeAllFinally() {
      window.axios.post('/admin/state/delete-multi', {
    ...
    getEntities() {
      let url = window.urlBuilder(this.materials, 'admin/state/index', this.options, this.headers)
    ...
    remove() {
      window.axios.delete('admin/state/delete/' + this.selectedEntityID).then(() => {
        this.getEntities()
      })
