...
export default {
  ...
  methods: {
    async removeAllFinally() {
      window.axios.post('/admin/tag/delete-multi', {
    ...
    getEntities() {
      let url = window.urlBuilder(this.tags, 'admin/tag/index', this.options, this.headers)
    ...
    remove() {
      window.axios.delete('admin/tag/delete/' + this.selectedEntityID).then(() => {
        this.getEntities()
      })
